...
- **Voice check, per AI-drafted piece.** Any body an AI drafts is checked against
  `personal-register/voice.md` before the approval gate — it must read as Vitalii, not as
  a pitch. The read-aloud test there is the bar. That file is private (see `context.md`);
  if it isn't present locally, this gate cannot run and drafting should stop and say so
